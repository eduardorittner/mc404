...
/* Prints log in the following format: ID <Target.x, Target.y, Target.z> (Car.x,
 * Car.y, Car.z) [Distance] Timestamp */
void print_log(int id, Node *checkpoint, int x, int y, int z, int distance,
               char *str) {
...
/* Go to target braking in small intervals until reaches it inside a radius of
 * 12 meters */
...
/* Stops engine and performs a turn to left or right until car is aligned with
 * target gyroscope angle */
...
int main() {
  ...
  /* Performs loop with go_to, print_log and action until last checkpoint is
   * reached */
  while (run) {
    go_to(&fill);
    get_position(&x, &y, &z);
    print_log(id, &fill, x, y, z, get_distance(x, y, z, fill.x, fill.y, fill.z),
              buffer);
