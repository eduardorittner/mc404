...
recursive_tree_search:
    # Searches a binary tree recursively until it finds the value #
    # If the value is not found, 0 must be returned #
    # INPUT #
    # a0 - root node #
    # a1 - value #
    # OUTPUT #
    # a0 - depth where value was found #
    ...
    beqz a0, not_found  # If current node is null, then not found
    
    # Check if current node has value
    lw t0, 0(a0)        # Load current value
    beq a1, t0, found   

    # Check if left subtree has value
    lw a0, 4(a0)            # Load left node
    jal recursive_tree_search
    bnez a0, tail_call      # If a0 != 0, then it's a tail call

    # Check if right subtree has value
    lw a0, 4(sp)            # Load current node
    lw a0, 8(a0)            # Load rigth node
    jal recursive_tree_search
    bnez a0, tail_call      # If a0 != 0, then it's a tail call
    j not_found

    # If the current node has value, return 1
found:
    li a0, 1
    lw ra, 0(sp)
    addi sp, sp, 8
    ...
    # If any of them has the value, return their return + 1
tail_call:
    addi a0, a0, 1
    lw ra, 0(sp)
    addi sp, sp, 8
