...
int get_immediate(char buffer[], int *read_chars) {
  ...
  while (
      !((buffer[curr] >= '0' && buffer[curr] <= '9') || buffer[curr] == '-')) {
    curr++;
  }
  *read_chars += curr;
  return dec_to_int(&buffer[curr], read_chars);
...
void get_inst_data(char inst[], InstData *data) {
  int opcode = 0, rd = 0, rs1 = 0, rs2 = 0, imm = 0, funct3 = 0, funct7 = 0;
  InstType type = I;
...
int main() {
  char linha[40];
  int n = read(STDIN_FD, linha, 40);

  int instr_hex = 0;
  InstData instr;
  get_inst_data(linha, &instr);

  pack(instr.opcode, 0, 6, &instr_hex);

  if (instr.type == R) {
    pack(instr.rd, 7, 11, &instr_hex);
    pack(instr.funct3, 12, 14, &instr_hex);
    pack(instr.rs1, 15, 19, &instr_hex);
    pack(instr.rs2, 20, 24, &instr_hex);
    pack(instr.funct7, 25, 31, &instr_hex);

  } else if (instr.type == I) {
    pack(instr.rd, 7, 11, &instr_hex);
    pack(instr.funct3, 12, 14, &instr_hex);
    pack(instr.rs1, 15, 19, &instr_hex);
    pack(instr.imm, 20, 31, &instr_hex);

  } else if (instr.type == S) {
    pack(instr.imm, 7, 11, &instr_hex);
    pack(instr.funct3, 12, 14, &instr_hex);
    pack(instr.rs1, 15, 19, &instr_hex);
    pack(instr.rs2, 20, 24, &instr_hex);
    pack((instr.imm >> 5), 25, 31, &instr_hex);

  } else if (instr.type == B) {
    pack((instr.imm >> 11), 7, 7, &instr_hex);
    pack((instr.imm >> 1), 8, 11, &instr_hex);
    pack(instr.funct3, 12, 14, &instr_hex);
    pack(instr.rs1, 15, 19, &instr_hex);
    pack(instr.rs2, 20, 24, &instr_hex);
    pack((instr.imm >> 5), 25, 30, &instr_hex);
    pack((instr.imm >> 12), 31, 31, &instr_hex);

  } else if (instr.type == U) {
    pack(instr.rd, 7, 11, &instr_hex);
    pack((instr.imm), 12, 31, &instr_hex);

  } else if (instr.type == J) {
    pack(instr.rd, 7, 11, &instr_hex);
    pack((instr.imm >> 12), 12, 19, &instr_hex);
    pack((instr.imm >> 11), 20, 20, &instr_hex);
    pack((instr.imm >> 1), 21, 30, &instr_hex);
    pack((instr.imm >> 20), 31, 31, &instr_hex);
  }
